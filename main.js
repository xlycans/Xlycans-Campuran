...
global.authFile = `${opts._[0] || 'Reel'}.data.json`
...
async function connectionUpdate(update) {
  ...
  setInterval(async () => {
	...
	"message": { "conversation": "Database Berhasil Dicadangkan" }}
    let sesi = await fs.readFileSync('./database.json')
    return await conn.sendMessage('120363141818160788@g.us', { document: sesi, mimetype: 'application/json', fileName: 'database.json' }, { quoted: q })
  }, 60 * 60 * 1000)

  
  if (global.db.data == null) await loadDatabase()
  console.log(JSON.stringify(update, null, 4))
  let iyah = pickRandom(['Asia','America','Europe','Australia','Africa'])
  if (update.receivedPendingNotifications) conn.sendMessage(`120363141818160788@g.us`, {text: `Bot Telah Online!\nServer:\n${iyah}\n\nKode:\n` + randomID(32) })
}
...
console.log(color(time,"yellow"),color("Connecting...","aqua"))
