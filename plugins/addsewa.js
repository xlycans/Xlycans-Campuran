let handler = async (m, { conn, text, usedPrefix, args, participants }) => {
  ...
  var name = m.sender
  var fkonn = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(m.chat ? { remoteJid: '6281211265326@s.whatsapp.net' } : {}) }, message: { contactMessage: { displayName: `${await conn.getName(name)}`, vcard: `BEGIN:VCARD\nVERSION:3.0\nN:;a,;;;\nFN:${name}\nitem1.TEL;waid=${m.sender.split('@')[0]}:${m.sender.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`}}}
...
Terimakasih telah mengundang *${m.name}*`, fkonn, {
    mentions: d
     }).then(async () => {
     await delay(5000)
     }).then( async () => {
...
handler.help = ['addsewa <chat.whatsapp.com> <day>']
handler.tags = ['owner']
handler.command = /^adds(ewa)?$/i
